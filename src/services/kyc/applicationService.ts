 ...
 */
export async function createApplication(input: CreateKycApplicationInput): Promise<string> {
  ...
  if (hasMint) {
    const ok = await verifyKycFeeViaMint(input.feeMintTransactionId!, input.userId);
    if (!ok) {
      throw new Error(
    ...
    if (!ok) {
      throw new Error('KYC fee payment could not be verified. Provide a valid fee tx hash.');
    }
  }
 ...
 */
async function enqueueWalletActivation(userId: string, stellarAddress: string): Promise<void> {
  try {
    ...
    logger.info('Wallet activation enqueued', { userId, stellarAddress: stellarAddress.slice(0, 8) + '…' });
  } catch (e) {
...
export async function addDocumentsAndEnqueue(
  ...
  userId: string,
  documents: { kind: string; storageRef: string; checksum?: string; mimeType?: string }[]
): Promise<void> {
 ...
 */
export async function transitionToMachineProcessing(applicationId: string): Promise<void> {
 ...
 */
export async function enqueueKycProcessing(applicationId: string): Promise<void> {
  try {
    const ch = getRabbitMQChannel();
    await ch.assertQueue(QUEUES.KYC_PROCESSING, { durable: true });
    ch.sendToQueue(QUEUES.KYC_PROCESSING, Buffer.from(JSON.stringify({ applicationId })), {
      persistent: true,
    });
  } catch (e) {
    logger.error('Failed to enqueue KYC processing', { applicationId, error: e });
    throw e;
  }
}
...
export async function approveApplication(applicationId: string): Promise<void> {
  ...
  logger.info('KYC application approved', { applicationId, userId: app.userId });
}
...
export async function rejectApplication(
  ...
  logger.info('KYC application rejected', { applicationId, userId: app.userId });
}
 ...
 */
export async function listApplicationsForUser(userId: string): Promise<
