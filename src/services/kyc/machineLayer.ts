...
export async function processApplication(applicationId: string): Promise<void> {
  ...
  if (!app || app.status !== 'machine_processing') {
    logger.warn('processApplication: app not found or not machine_processing', { applicationId });
    return;
  }
  ...
  if (PROVIDER === 'none') {
    redacted = { hints: ['machine_provider_disabled'], unreadableRegions: ['all'] };
    confidence = 0;
