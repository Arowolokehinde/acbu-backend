...
export function verifyFlutterwaveSignature(
  ...
  try {
    if (crypto.timingSafeEqual(Buffer.from(received, 'hex'), Buffer.from(computed, 'hex'))) {
      next();
      return;
    }
  } catch {
...
export async function handleFlutterwaveWebhook(
  ...
  try {
    ...
    logger.info('Flutterwave webhook received', { eventType, tx_ref: data.tx_ref, status: data.status });
