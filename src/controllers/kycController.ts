...
import { createApplication, addDocumentsAndEnqueue, getApplicationForUser, listApplicationsForUser } from '../services/kyc/applicationService';
import { getTasksForValidator, submitValidationResult } from '../services/kyc/validatorPool';
...
export async function postApplications(
  ...
  try {
    ...
    res.status(201).json({ application_id: applicationId, status: 'machine_processing' });
  } catch (e) {
    next(e);
  }
}
...
export async function getApplications(
  ...
  try {
    ...
    res.json({ applications: list.map((a) => ({ id: a.id, status: a.status, created_at: a.createdAt })) });
  } catch (e) {
    next(e);
  }
}
...
export async function getUploadUrls(
  ...
  try {
    const userId = req.apiKey?.userId;
    if (!userId) throw new AppError('User-scoped API key required', 401);
    const { applicationId, kind } = req.query as { applicationId?: string; kind?: string };
    if (!applicationId || !kind || !['id_front', 'id_back', 'selfie'].includes(kind)) {
      throw new AppError('applicationId and kind (id_front|id_back|selfie) required', 400);
    }
    ...
    const { url, key: k } = await getPresignedUploadUrl(key, 'application/octet-stream');
...
export async function postValidatorRegister(
  ...
  try {
    ...
    if (!user || user.kycStatus !== 'verified') {
      throw new AppError('KYC verification required to register as validator', 403);
    }
...
export async function getValidatorTasks(
  ...
  try {
    ...
    if (!user || user.kycStatus !== 'verified') {
      throw new AppError('KYC verification required to fetch validator tasks', 403);
    }
