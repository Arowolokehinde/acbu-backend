 ...
 */
export async function createRewardsForApplication(applicationId: string): Promise<void> {
  ...
  logger.info('KYC validator rewards created', { applicationId, count: validations.length });
}
...
export async function processPendingRewards(): Promise<void> {
  ...
  for (const r of pending) {
    ...
    logger.debug('Reward pending payout', { rewardId: r.id, validatorId: r.validatorId });
  }
}
